...
namespace Company
{
    public partial class ReportCompanyPurchaseDaily : Form
    {
        ...
        public ReportCompanyPurchaseDaily(CompanyBusiness CB)
        {
            InitializeComponent();
            ...
            if(EndDate>BeginDate){
                if (BeginDate > MinDate && BeginDate < MaxDate && EndDate > MinDate && EndDate < MaxDate)
                {
                    ...
                    if (difdate.Days <= 30)
                    {
                        ...
                        CompanyChart_DataSet = Company.DayBase_AllChargesInWhole(BeginDate, EndDate);
