...
namespace Company
{
    public partial class ReportAgencySoldDaily : Form
    {
        ...
        public ReportAgencySoldDaily(CompanyBusiness CB,Int64 AG_ID)
        {
            InitializeComponent();
        ...
        public void Tab1Fill(DateTime BeginDate,DateTime EndDate)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DateTime BeginDate = faDatePicker1.SelectedDateTime;
            DateTime EndDate = faDatePicker2.SelectedDateTime;
            EndDate = EndDate.AddDays(1); // for being in range of date truly in database
            if(EndDate>BeginDate){
                if (BeginDate > MinDate && BeginDate < MaxDate && EndDate > MinDate && EndDate < MaxDate)
                {
                    System.TimeSpan difdate = EndDate.Subtract(BeginDate);
                    if (difdate.Days <= 30)
                    {

                        CompanyChart_DataSet = Company.DayBase_AgencySoldDaily(BeginDate, EndDate,AgencyID);
                        Tab1Fill(BeginDate, EndDate);
                        Tab2Fill();
                        Tab3Fill();
                        Tab4Fill();
                        Tab5Fill();
                    ...
                    else
                    {
                        ...
                        pmb.show("تاریخ شروع تا تاریخ پایان نباید بیشتر از 30 روز باشد.", "هشدار", PersianMessageBox.Warning, 1, "YES/NO");
                    }
                }
                else {
                    PersianMessageBox pmb = new PersianMessageBox();
                    pmb.show("تاریخ شروع بایستی از تاریخی قبل از تاریخ پایان باشد..", "هشدار", PersianMessageBox.Warning, 1, "YES/NO");
                }
            }
            else {
                PersianMessageBox pmb = new PersianMessageBox();
                pmb.show("تاریخ وارد شده یا خارج از محدوده مجاز است و یا ه درستی وارد نشده است.", "خطا", PersianMessageBox.Error, 1, "YES/NO");
            }
        ...
        {
            ReportAgencySoldDaily_PrintPage RCPD_PP = new ReportAgencySoldDaily_PrintPage(CompanyChart_DataSet.Tables["WholeData_DT"]);
            RCPD_PP.Show();
        }
